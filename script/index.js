...
formSubmitBtn.addEventListener("click", (event) => {
  ...
  event.preventDefault();
  maxWorkload = maxWorkingHoursInput.value
    ? Number(maxWorkingHoursInput.value)
...
function encodeRows() {
  ...
  for (let row = 1; row < rows.length; row++) {
    ...
    for (let col = 1; col < rows[0].length; col++) {
      // c: index of work day
      const availableShifts = rows[row][col]
        .split(";")
...
function displayFileSummary() {
  ...
  for (const interval of shiftIntervals) {
    const li = document.createElement("li");
    li.textContent = `${String(interval[0]).padStart(2, "0")} - ${String(
      interval[1]
    ).padStart(2, "0")}`;
  ...
  workingHourPara.textContent =
    "Based on the shift information, your working hours:";
...
function displaySchedule() {
  ...
  for (let employeeId = 0; employeeId < employees.length; employeeId++) {
    ...
    for (let dayId = 0; dayId < dayLabels.length; dayId++) {
      ...
      td.textContent =
        shiftId != -1
          ? `${String(shiftIntervals[shiftId][0]).padStart(2, "0")} - ${String(
              shiftIntervals[shiftId][1]
            ).padStart(2, "0")}`
          : "N/A";
      row.appendChild(td);
    }
